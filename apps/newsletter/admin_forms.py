...
def make_subscription(newsletter, email, name=None):
    if Subscription.objects.filter(
                                ...
                                subscribed=True,
                                email_field__exact=email).count():
        return None
...
def check_email(email, ignore_errors=False):
    ...
    if len(email) <= email_length or ignore_errors:
        ...
        raise forms.ValidationError(
                _("E-mail address %(email)s too\
                long, maximum length is\
...
class NewsletterAdminForm(forms.ModelForm):
    ...
    start_event = forms.DateField(widget=forms.DateInput())
    end_event = forms.DateField(widget=forms.DateInput())
    start_action = forms.DateField(widget=forms.DateInput())
    end_action = forms.DateField(widget=forms.DateInput())
    start_photoreports= forms.DateField(widget=forms.DateInput())
    end_photoreports = forms.DateField(widget=forms.DateInput())
    start_newsitems = forms.DateField(widget=forms.DateInput())
    end_newsitems = forms.DateField(widget=AdminDateWidget())
...
def create_admin_newsletter_form(stage):
    ...
    if stage:
        return forms.models.modelform_factory(Newsletter, form=NewsletterAdminForm, exclude=fields)
    else:
        return forms.models.modelform_factory(Newsletter, form=NewsletterAdminForm, fields=fields)
