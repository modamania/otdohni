...
GROUP BY object_id""" % (
    ...
    def record_vote(self, obj, user, vote):
        ...
        #send signal to updates info about item
        user_voted.send(self, user=user, voted_item=obj)

    def get_top(self, Model, objects=None, limit=10, reversed=False):
        """
...
HAVING AVG(vote) > 0""" % connection.ops.quote_name(self.model._meta.db_table)
        ...
        if objects:
            objects = objects.in_bulk([id for id, score in results])
        else:
...
class Vote(models.Model):
    user = models.ForeignKey(User, verbose_name=_(u'user'), related_name='votes')
