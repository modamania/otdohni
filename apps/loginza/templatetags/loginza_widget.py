...
def _return_path(request, path=None):
    if path is not None and path not in settings.AMNESIA_PATHS:
        if 'next' in request.session:
            ...
            request.session['loginza_return_path'] = next
...
def return_vote_url(poll_id, work_id):
    return urllib.quote(_absolute_url(
                reverse('loginza_vote_return',
                        args=[int(poll_id), int(work_id)])), '')
...
style="width:380px;height:200px;" scrolling="no" frameborder="no"></iframe>""" % {
...
def vote_template(kwargs, caption):
    return """<script src="http://loginza.ru/js/widget.js" type="text/javascript"></script>
<a href="http://loginza.ru/api/widget?%(providers)slang=%(lang)s&token_url=%(return-url)s" class="loginza">
    %(caption)s
</a>""" % {
        'return-url': return_vote_url(kwargs['poll'], kwargs['work']),
        'caption': caption,
        'lang': kwargs['lang'],
        'providers': providers(kwargs),
        'poll': kwargs['poll'],
...
@register.tag
def loginza_vote(parser, token):
    return _loginza_widget(parser, token, vote_template)
