    ...
    def for_identity(self, identity, request):
        try:
            ...
            if request.user.is_authenticated():
                ...
                if not username:
                    username = 'user%s' % loginza_data.get('uid', None)

                name = loginza_data.get('name', None)
                if name:
                    first_name = getattr(name, 'first_name', '')
                    last_name = getattr(name, 'last_name', '')
                else:
                    first_name = ''
                    last_name = ''
                ...
                user.save()
            user_map = UserMap.objects.create(identity=identity, user=user,verified=True)
